...
function normalizeOptions(advancedOptions) {
  ...
  overwriteArray(advancedOptions, 'modulePathIgnorePatterns', { disableResolvePath: true })
  overwriteMap(advancedOptions, 'moduleNameMapper', { disableResolvePath: true })
  overwriteValue(advancedOptions, 'globalSetup')
  overwriteArray(advancedOptions, 'transformIgnorePatterns', { disableResolvePath: true })
...
const defaultAdvancedOptions = {
  ...
  browser: false,
  modulePaths: process.env.NODE_PATH ? process.env.NODE_PATH.split(nps.delimiter) : []
}
...
function runScriptAdvanced(code, advancedOptions = {}) {
  ...
  if (advancedOptions.preset) {
    ...
    if (typeof advancedOptions.preset === 'string') {
      preset = resolvePath(angleBracketsReplacer(preset, parameters), {
        basedir: advancedOptions.rootDir,
        fs: advancedOptions.fs
      })
    }
    if (preset && preset.name) {
      preset = {
        ...preset,
        name: resolvePath(angleBracketsReplacer(preset.name, parameters), {
          basedir: advancedOptions.rootDir,
          fs: advancedOptions.fs
        })
      }
...
function runScriptAdvancedFile(filename, advancedOptions) {
  advancedOptions = normalizeOptions(advancedOptions)
  filename = nps.resolve(advancedOptions.rootDir, filename)
