...
const { SourceMapTree } = require('./utils/source-map')
...
function runScript(code, options) {
  ...
  if (enableSourceMap && !sourceMapTree && options.filename) {
    sourceMapTree = new SourceMapTree({ fs, baseDir: nps.dirname(options.filename) })
    const lines = HEAD_WRAP.split('\n')
    sourceMapTree.offset = {
  ...
  if (enableSourceMap && options.filename) {
    sourceMapTree.tree.set(options.filename, {
      map,
      code,
